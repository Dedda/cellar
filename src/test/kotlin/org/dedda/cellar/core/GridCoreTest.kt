...
internal class GridCoreTest {
    ...
    @Test
    fun nonFunctionContentRendersNormally() {
        val gridCore = GridCore()
        val structsPostfix = "${this.javaClass.canonicalName}::nonFunctionContentRendersNormally"
        val folderId = "Folder $structsPostfix"
        val tableId = "Table $structsPostfix"
        ...
        assertThat(gridCore.tableIdsForFolderId(folderId)).contains(tableId)
        assertThat(gridCore.renderedAt(folderId, tableId, 3, 4)).isEqualTo("Hello there!")
    }
}